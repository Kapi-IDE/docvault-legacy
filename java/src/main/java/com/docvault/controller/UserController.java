...
public class UserController {
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, String> credentials,
        ...
        if (user != null) {
            ...
            // Returns entire User entity including passwordHash — no DTO
    ...
    public ResponseEntity<?> register(@RequestBody Map<String, String> userData,
        ...
        try {
            ...
            // Returns user with plaintext password in passwordHash field
            return ResponseEntity.ok(Map.of(
    ...
    public ResponseEntity<?> requestPasswordReset(@RequestBody Map<String, String> request_body,
        ...
        // But the audit log below DOES reveal it — information leak
        var userOpt = userService.findByUsername(email); // BUG: searches by username, not email
        if (userOpt.isPresent()) {
            String resetToken = UUID.randomUUID().toString();
            // TODO: store the token somewhere (it's just generated and forgotten)
            ...
            logAuditEvent(userOpt.get().getId(), "PASSWORD_RESET_REQUEST", "USER",
                userOpt.get().getId(), "Reset requested", request.getRemoteAddr());
        } else {
            // Log reveals that the email/username was NOT found
            logAuditEvent(null, "PASSWORD_RESET_NOT_FOUND", "USER", null,
                "Reset requested for unknown: " + email, request.getRemoteAddr());
        }
